...
import { getUserActivityLogs } from 'apiSdk/user-activity-logs';
import { UserActivityLogInterface } from 'interfaces/user-activity-log';
import { getOrganizationMemberships } from 'apiSdk/organization-memberships';
...
import { CallRecordingListPage as ShowCaseList } from 'pages/call-recordings';

function DashboardPage() {
  const { session } = useSession();

  const userActivityLogStat = useSWR<PaginatedInterface<UserActivityLogInterface>>(
    () => `/user-activity-logs`,
    () => getUserActivityLogs({ limit: 1, offset: 0 }),
  );

  const organizationMembershipStat = useSWR<PaginatedInterface<OrganizationMembershipInterface>>(
    () => `/organization-memberships`,
    () => getOrganizationMemberships({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: userActivityLogStat?.data?.totalCount,
      isLoading: userActivityLogStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Organization Memberships',
      result: organizationMembershipStat?.data?.totalCount,
      isLoading: organizationMembershipStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'User Profiles',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`https://calendly.com/app/scheduled_events/user/me`} passHref legacyBehavior>
                {/* <NextLink href={`call-recordings`} passHref legacyBehavior> */}
                  <Button
